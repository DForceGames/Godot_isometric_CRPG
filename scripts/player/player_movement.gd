...
func _is_cell_walkable(map_coords: Vector2i) -> bool:
	...
	# Check for collision on obstacle layers, but only if the tileset has a physics layer
	for obstacle_layer in obstacle_layers:
		if obstacle_layer.tile_set and obstacle_layer.tile_set.get_physics_layers_count() > 0:
	...
	# Use path_has_obstacles and evaluate_path_quality for additional checks
	if path_has_obstacles([map_coords]):
		return false
...
func set_target_position(pos: Vector2) -> void:
	...
	# Check if target is walkable and within bounds
	if not astar_grid.region.has_point(target_map_coords) or astar_grid.is_point_solid(target_map_coords):
		# Find nearest walkable tile
		target_map_coords = find_nearest_walkable_tile(target_map_coords)
	
	var new_path = astar_grid.get_id_path(current_map_coords, target_map_coords)
...
func path_has_obstacles(path: Array[Vector2i]) -> bool:
	# Check if the given path encounters any obstacles
	for cell in path:
...
func evaluate_path_quality(path: Array[Vector2i]) -> Dictionary:
	...
	var tight_spaces = 0
	var result = {"has_obstacles": false, "obstacles_count": 0, "tight_spaces": 0}
...
func is_position_walkable(pos: Vector2) -> bool:
	...
	for obstacle_layer in obstacle_layers:
		var cell_source_id = obstacle_layer.get_cell_source_id(tile_pos)
		if cell_source_id != -1:  # If there's a tile here
			return false
	
	# Check if the position collides with any object in the physics world
...
func find_path(start_pos: Vector2, end_pos: Vector2) -> Array:
	...
	var _start_tile = primary_tilemap_layer.local_to_map(primary_tilemap_layer.to_local(start_pos))
	var _end_tile = primary_tilemap_layer.local_to_map(primary_tilemap_layer.to_local(end_pos))
	...
	if not is_position_walkable(end_pos):
		...
		for direction in directions:
			...
			if is_position_walkable(new_end_pos):
				end_pos = new_end_pos
				_end_tile = primary_tilemap_layer.local_to_map(primary_tilemap_layer.to_local(new_end_pos))
				print("PlayerMovement: Found walkable position at", new_end_pos)
				break

	# Example: return an empty array if no pathfinding is implemented yet
	return []
